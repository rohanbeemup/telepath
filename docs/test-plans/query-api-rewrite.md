...
- **Third pass, on the turn accounting:** an init that merely confirmed a turn a send had
  begun zeroed the count of a message sent in between, so the wrap-up confirmation and
  the daemon status under-reported what was still unanswered; the cut-short masking
  applied to `after-turn` too, where the turn runs to its end and its error is real; and
  the wrap-up's verdict sent the binding's stored hand-off, so a wrap-up that wrote none
  offered an earlier one as new. The wrap state now carries its mode and its own hand-off.
...
| `messages sent during a turn are folded into it and one result answers them all` | four sends during one turn report 1 to 4 in flight; an init that only confirms a turn a send began keeps the count of what was sent before it; the one result ends the turn with 0 in flight and clears the queue; output arriving with no turn known begins one | a per-message count that drifts by one per folded message and reads "9 messages queued" over an idle session, or an init that zeroes a message still unanswered |
...
| `the turn a wrap-up cuts short is not reported as an error, and a second result completes the wrap-up without a hand-off` | the error result a `now` message forces mid-thought reaches the bot as a quiet turn end; with `after-turn` the finished turn's error stays reported; the wrap-up completes on its second result whether or not a hand-off was written, offering only a hand-off this wrap-up wrote (an earlier one stored on the binding is neither offered nor touched); a later ordinary error is still reported | an "⚠️ turn ended" notice for every wrap-up, a real error hidden behind a finish-first wrap-up, a stale hand-off offered as new, or a wrap-up that never completes and blocks the next one |
