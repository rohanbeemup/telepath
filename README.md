...
| 🔎 Activity feed | Whether the turn's live status message (below) also lists what the session did: 🖥 commands and 🤖 subagents by their description, ✏️ edits collapsed to one line with a count and a few file names, reads and searches as a count. On by default in ⚡ auto, where no approval prompts show what the session is doing; off by default under approvals. Typed: `feed on` / `feed off`. |

Every turn gets **one live status message**: `⏳ Working · 1:42 · 9 tool calls`, edited in place at most every 12 seconds (only when something changed), with the typing indicator alongside. It stays the **last message in the topic**: when Claude posts something below it, it is deleted and re-posted silently at the bottom, once per burst. Telegram's rate limit of about twenty operations a minute applies per group, and all topics share one group, so every status operation across every topic draws from one budget (twelve a minute, the rest is kept for real messages); the edit interval stretches as more topics run at once, typing slows above three, and every API call waits out a 429 for exactly the `retry_after` Telegram names instead of failing. It never becomes the answer, because an edit sends no notification: answers, questions and approvals are always new messages. When the turn ends the status becomes a one-line summary (`✅ Done · 3:12 · 14 tool calls · 7 files edited`), or disappears if the turn was short and touched nothing. While a rate limit holds the turn it reads `⏸ Rate limit hit · resets at 07:10`; if you send more messages while a turn runs it shows `· 2 messages queued` until that turn's result, which answers them too (the CLI folds messages sent mid-turn into the running turn). No percentage is shown: none exists for open-ended work, and a bar that crawls to 90% and stalls is worse than an honest clock. The status carries a **⏹ Wrap up** button (also `wrap up` or `stop` typed): the session finishes the step it is on, writes a hand-off (done · open · how to resume) and stops, with nothing interrupted mid-step; **▶️ Resume** then continues from that hand-off. Messages you sent during the turn are already with the session, so the hand-off covers them; there is nothing to drop.
