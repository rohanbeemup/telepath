...
Every turn gets **one live status message**: `⏳ Working · 1:42 · 9 tool calls`, edited in place at most every 12 seconds (only when something changed, and backing off exactly as long as Telegram asks on a 429), with the typing indicator alongside. It never becomes the answer, because an edit sends no notification: answers, questions and approvals are always new messages. When the turn ends the status becomes a one-line summary (`✅ Done · 3:12 · 14 tool calls · 7 files edited`), or disappears if the turn was short and touched nothing. While a rate limit holds the turn it reads `⏸ Rate limit hit · resets at 07:10`; if you send more messages while a turn runs it shows `· 2 messages queued`, so you know they were taken. No percentage is shown: none exists for open-ended work, and a bar that crawls to 90% and stalls is worse than an honest clock.
